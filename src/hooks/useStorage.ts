...
// Importação do tipo Session, que é necessário
import { Product, Sale, Customer, InstallmentSale, SaleItem, Session } from '../types';
...
  
    // ✅ NOVO ESTADO: Armazena a sessão do Supabase. O fetchAllData dependerá disso.
    const [session, setSession] = useState<Session | null>(null);
  const [loading, setLoading] = useState(true); // O estado de loading inicial agora será controlado pelo useEffect de auth
...
    const chars = '0123456789'; // Note: 'I', 'O', 'L' foram removidos.
    const length = 6; // Comprimento desejado do SKU
    let result = '';
...
    
    // ✅ CORREÇÃO 1: Checa a sessão diretamente do estado, garantindo que o usuário está logado
    if (!session?.user) { 
        setLoading(false);
        return;
    }
...
      // ❌ LINHA REMOVIDA: Era a causa do problema de timing:
...
      setLoading(false);
    }
  }, [session]); // ✅ CORREÇÃO 2: fetchAllData agora depende do estado local 'session'
...
  // ✅ NOVO EFFECT: Monitora a autenticação e aciona fetchAllData
  useEffect(() => {
        // Tenta buscar a sessão atual para o carregamento inicial
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session);
    });
    
    // Configura o listener para mudanças de estado (login/logout)
...
        
        if (!currentSession) {
            // Limpar estados em caso de logout
            setProducts([]);
            setSales([]);
            setCustomers([]);
            setInstallmentSales([]);
...
  }, []); // Array de dependências vazio para rodar apenas uma vez
    ...
    // ✅ NOVO EFFECT: Dispara fetchAllData quando a sessão muda (e está presente)
    useEffect(() => {
        if (session) {
            fetchAllData();
        } else if (session === null) {
            // Se a sessão for explicitamente nula após o carregamento, garantimos que loading = false
            setLoading(false);
        }
    }, [session, fetchAllData]); // fetchAllData está aqui porque é uma dependência, mas a mudança principal é 'session'
